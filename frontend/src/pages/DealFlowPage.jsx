  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <KPICard label="GROSS ORIGINATIONS (Q)" value={latestTrend?.total_new_originations_bn} format="currency" accentColor="#00C8E0" loading={trendsLoading} />
        ...
        {trendsLoading ? <LoadingSpinner /> : (
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={trends ? [...trends].reverse() : []} margin={{ top: 20, right: 20, left: -20, bottom: 0 }}>
              ...
              <YAxis yAxisId="left" stroke="#8899AE" tick={{ fill: '#8899AE', fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={(val) => `$${val}B`} />
              <YAxis yAxisId="right" orientation="right" stroke="#8899AE" tick={{ fill: '#8899AE', fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={(val) => `$${val}B`} />
              <Tooltip content={<CustomTooltip />} />
              <Legend iconType="circle" wrapperStyle={{ fontSize: '11px' }} />
              <Bar yAxisId="left" dataKey="total_new_originations_bn" name="Originations" fill="#00C8E0" radius={[4, 4, 0, 0]} />
              <Bar yAxisId="left" dataKey="total_repayments_bn" name="Repayments" fill="#8B5CF6" radius={[4, 4, 0, 0]} />
          ...
          {sectorLoading ? <LoadingSpinner /> : (
            <ResponsiveContainer width="100%" height="100%">
              <BarChart layout="vertical" data={bySector || []} margin={{ top: 0, right: 10, left: 40, bottom: 0 }}>
                ...
                <XAxis type="number" stroke="#8899AE" tick={{ fill: '#8899AE', fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={(val) => `$${val}M`} />
                <YAxis dataKey="industry" type="category" stroke="#8899AE" tick={{ fill: '#8899AE', fontSize: 10 }} axisLine={false} tickLine={false} width={120} />
                <Tooltip content={<CustomTooltip />} formatter={(val) => `$${val}M`} />
                <Bar dataKey="fair_value_mm" name="Origination Vol" fill="#00C8E0" radius={[0, 4, 4, 0]} barSize={16} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </ChartPanel>
          ...
          {holdLoading ? <LoadingSpinner /> : (
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={holdSizes ? [...holdSizes].reverse() : []} margin={{ top: 20, right: 10, left: -20, bottom: 0 }}>
                ...
                <YAxis stroke="#8899AE" tick={{ fill: '#8899AE', fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={(val) => `$${val}M`} />
                <Tooltip content={<CustomTooltip />} />
                <Legend iconType="circle" wrapperStyle={{ fontSize: '11px' }} />
                <Line type="monotone" dataKey="avg_loan_size" name="Average Hold" stroke="#00C8E0" strokeWidth={3} dot={{ r: 4 }} />
